<template>
  <div class="flex min-h-[400px] h-full shadow-[0_2px_12px_0_rgb(0_0_0/10%)]">
    <div v-show="showMode !== 2" class="flex-1 flex-shrink-0 flex flex-col pl-4 pb-4 w-1/2 h-full overflow-hidden">
      <h2>
        Markdown <CommonButton icon class="font-normal text-base" @click="showMode = showMode ? 0 : 1">
          <IconReturn v-if="showMode" class="w-4 h-4" />
          <IconView v-else class="w-4 h-4" />
        </CommonButton>
      </h2>
      <textarea
        class="w-full h-full pr-4 overflow-y-auto resize-none border-none outline-none" :value="modelValue"
    ...
    >
      <h2>
        预览 <CommonButton icon class="font-normal text-base" @click="showMode = showMode ? 0 : 2">
          <IconReturn v-if="showMode" class="w-4 h-4" />
          <IconView v-else class="w-4 h-4" />
        </CommonButton>
      </h2>
